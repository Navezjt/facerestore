...
class CropFace:
    ...
    def crop_face(self, image, facedetection, index):
        device = model_management.get_torch_device()
        if self.face_helper is None:
            self.face_helper = FaceRestoreHelper(1, face_size=512, crop_ratio=(1, 1), det_model=facedetection, save_ext='png', use_parse=True, device=device)
        ...
        if index < 0 or index >= faces_found:
            return (image,faces_found)

        cropped_face_1 = self.face_helper.cropped_faces[index]
        ...
        cropped_face_6 = np.array(cropped_face_5).astype(np.float32) / 255.0
        cropped_face_7 = torch.from_numpy(cropped_face_6).unsqueeze(0)
        return (cropped_face_7,faces_found)
